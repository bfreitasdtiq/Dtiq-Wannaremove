...
cls
#Checking for Temp Folder, if it doesn't exist it will be created.
$path = "C:\temp\Dtiq-wanna"
If(!(test-path $path))
{
      New-Item -ItemType Directory -Force -Path $path
      Write-host "Folder Created C:\temp\dtiq-wanna\"
}

#Logging Logic Started
$hostname = hostname
Start-Transcript -path C:\temp\dtiq-wanna\DTIQ-WannaRemove-$hostname.log -append

Write-Host Dtiq-Wannaremove Starting Search
Write-Host Dtiq-Wannaremove Searching for in3.ps1
...
#Running Remove-Item (Remove / add Whatif to stop deletion the keys)
